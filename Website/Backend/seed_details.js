...
async function seed() {
  console.log("Seeding product details, images, specs, features...");
  ...
  // Clean old details to avoid primary key/unique violations
  await execute("DELETE FROM product_details");
  await execute("DELETE FROM product_images");
  await execute("DELETE FROM product_specs");
  await execute("DELETE FROM product_features");
  ...
  for (const p of products) {
    ...
    await execute(
      "INSERT INTO product_details (product_id, description, full_specs, warranty_info, shipping_info) VALUES (?, ?, ?, ?, ?)",
      [p.product_id, desc, fullSpecs, warranty, shipping]
    );

    // 2. Insert Images (Primary + 2 extra)
    const imgList = IMAGES[cat] || IMAGES["Electronics"];
    for (let i = 0; i < imgList.length; i++) {
      const isPrimary = i === 0 ? 1 : 0;
      // append cache-buster/random query parameter to make them slightly unique visually
      const imgUrl = `${imgList[i]}&sig=${p.product_id}_${i}`;
      await execute(
        "INSERT INTO product_images (product_id, image_url, is_primary) VALUES (?, ?, ?)",
        [p.product_id, imgUrl, isPrimary]
      );
    }
    ...
    for (const spec of specs) {
      await execute(
        ...
        [p.product_id, spec.key, spec.val]
      );
    }
